...
export class MCPAuthenticator {
  ...
  private printTokenInstructions(token: string): void {
    ...
    if (isTty || forcePrint) {
      ...
      log.info("  ╠══════════════════════════════════════════════════════════════════════╣");
      log.info("  ║  Add to your stdio MCP client config (Claude Code / Codex / etc):   ║");
      log.info(`  ║    NLMCP_AUTH_TOKEN=${token.padEnd(47)}║`);
      log.info("  ║    NLMCP_STDIO_TRANSPORT_AUTH=true                                  ║");
      log.info("  ║                                                                      ║");
      log.info("  ║  Or as a one-liner for Claude Code:                                 ║");
      log.info(`  ║    claude mcp add notebooklm \\${" ".repeat(37)}║`);
      log.info(`  ║      --env NLMCP_AUTH_TOKEN=${token} \\${" ".repeat(Math.max(0, 36 - token.length))}║`);
      log.info("  ║      --env NLMCP_STDIO_TRANSPORT_AUTH=true \\                        ║");
      log.info(`  ║      -- npx notebooklm-mcp-secure${" ".repeat(34)}║`);
      log.info("  ║                                                                      ║");
      log.info("  ║  Why both env vars? See README → MCP auth modes (stdio).            ║");
