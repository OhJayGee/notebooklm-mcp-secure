...
**Latest: v2026.3.5** — Round 4 external review fixes (Claude + Codex + Gemini). Tests: 804 → 837. 8 distinct findings (after deduplication across 3 reviewers) including a CRITICAL SSRF in audio download.

| Version | Highlights |
|---------|------------|
| **v2026.3.5** | **Round 4 external review** — Three independent reviewers (Claude Opus 4.7 + Codex GPT-5 + Gemini 3.1 Pro) ran the Template 2 prompt against v2026.3.4. CRITICAL SSRF in `audio-manager.downloadAudio` (`page.goto` on DOM-scraped URL) closed via new shared `url-validation.ts` module. HIGH audit-log hash-chain race condition fixed by stamping the chain link inside the file-lock critical section. Library mutations now audit-logged; shutdown handler flushes audit + query loggers; alert-manager / SIEM-exporter validate outbound URLs; quota schema validation on load; `SettingsManager.saveSettings` serialised; `get_health.deep_check` gated on env-var opt-in. 33 new regression tests. |
| **v2026.3.4** | **Whole-repo external review** — Codex + Gemini 3.1 Pro ran the toned-down Template 2 prompt against the whole codebase. 11 distinct findings: `list_webhooks` redaction (Slack/Discord URL secrets), MCP resource-read auth gap, `run_health_check` admin-gating, `export_library` migration to shared path-policy, response-validator coverage for Gemini + chat-history, `validateSessionId`/`validateQuestion`/`validateSourceUrl` consistency, WebhookDispatcher init race, stack-frame stripping in `getSanitizedErrorMessage`, credential wipe on shutdown. 27 new regression tests. |
